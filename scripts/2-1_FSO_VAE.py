...
train_dataset = get_dataset(train_files).batch(batch_size, drop_remainder=True)
val_dataset = get_dataset(val_files).batch(batch_size, drop_remainder=True)
...
with strategy.scope():
    ...
    # autoencoder = Model(x, cnn_encoding)
    ...
    csv_logger = tf.keras.callbacks.CSVLogger(
        filename = model_dir.joinpath(f"training/FSO_VAE-ep{epochs}-bs{batch_size}-v1.csv")
    )

    # history = autoencoder.fit(x = {
    #     "tf_input": x_train_tf,
    #     "dist_input": train_dist[:, :distribution_dim],
    #     },
    #     y = {
    #     "TF_output": y_train_tf,
    #     "dist_output": train_dist[:, :distribution_dim],
    #     },
    #     epochs=epochs, batch_size=batch_size,
    #       validation_data=[{"tf_input": x_val_tf, "dist_input": val_dist[:, :distribution_dim]},
    #                                                     {"TF_output": y_val_tf, "dist_output": val_dist[:, :distribution_dim]}],
...
# val_loss = str(np.round(history['val_loss'][epochs - 1], 0))
    autoencoder.save_weights(model_dir.joinpath(f"training/FSO_VAE-weights-ep{epochs}-bs{batch_size}-v1.h5"))
