...
if __name__ == "__main__":
    ...
    result.reset_index(drop=True).to_feather(
        path.joinpath(
            "functions_simple_10_numerics_Distribution_indiv_scale_allBasins_2.feather"
        )
    )
    ...
    tf_dist.index = generator_data.index
    ...
    tf_dist.reset_index(drop=True).to_feather(
        path.joinpath(
            "functions_simple_10_numerics_Distribution_indiv_scale_wrecalc_allBasins_no_extremes_2.feather"
        )
    )
    generator_data.reset_index(drop=True).to_feather(
        path.joinpath(
            "generator_data_simple_10numerics_wrecalc_allBasins_no_extremes_2.feather"
        )
    )
