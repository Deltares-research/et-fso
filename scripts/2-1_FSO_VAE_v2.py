...
index2word = (
    pd.read_feather(data_dir.joinpath("index2word_10numerics.feather"))
    .iloc[0]
    .astype(str)
    .to_list()
)
...
train_dataset = (
    get_dataset(train_files).shuffle(1).batch(batch_size, drop_remainder=True)
)
...
class VAE:
    ...
    def get_model(self):
        ...
        self.z_mean = Dense(latent_dim, activation="linear", name="z_mean")(
            cnn_encoding
        )
        self.z_log_var = Dense(latent_dim, activation="linear", name="z_log_var")(
            cnn_encoding
        )
    ...
    def sampling(self, args):
        ...
        epsilon = K.random_normal(
            shape=(K.shape(self.z_mean)[0], latent_dim), mean=0.0, stddev=epsilon_std
        )
...
with strategy.scope():
    ...
    csv_logger = tf.keras.callbacks.CSVLogger(
        filename=model_dir.joinpath(
            f"training/FSO_VAE-ep{epochs}-bs{batch_size}-v3.csv"
        )
    )
    ...
    vae.model.save_weights(
        model_dir.joinpath(f"training/FSO_VAE-weights-ep{epochs}-bs{batch_size}-v3.h5")
    )
